...
class Data_3_hr
{
    ...
    {
        $commands = [
            'CREATE TABLE IF NOT EXISTS \'3_hr\'(
                        	            voltage	REAL,
	                                    current	REAL,
	                                    power	REAL,
	                                    energy	REAL,
	                                    time TEXT NOT NULL UNIQUE)',
            'CREATE INDEX IF NOT EXISTS time_index ON \'3_hr\' (time);'
        ];
        //var_dump($commands);
    ...
    {
        $sql = "INSERT or REPLACE INTO '3_hr' VALUES(:voltage,:current,:power,:energy,:time)";
    ...
    public static function getLast($esp_id, $limit = 1, $columns = '*')
    {
        ...
        $sql = "SELECT $columns FROM '3_hr' ORDER BY time DESC LIMIT {$limit}";
    ...
    public static function getAvDay($esp_id, $time){
        $pdo = new \PDO("sqlite:" . $_SERVER['DOCUMENT_ROOT'] . "/db/$esp_id.db");
        $sql = "SELECT printf('%.1f',avg(voltage)),printf('%.3f',avg(current)),printf('%.1f',avg(power)),printf('%.3f',max(energy)-min(energy))
                FROM '3_sec' 
