...
class Data_3_day_v10
{
    ...
    {
        $commands = [
            'CREATE TABLE IF NOT EXISTS \'3_day\'(
                        	            voltage	REAL,
	                                    current	REAL,
	                                    power	REAL,
	                                    energy	REAL,
	                                    time TEXT NOT NULL UNIQUE)',
            'CREATE INDEX IF NOT EXISTS time_index ON \'3_day\' (time);'
        ];
        //var_dump($commands);
    ...
    {
        $sql = "INSERT or REPLACE INTO '3_day' VALUES(:voltage,:current,:power,:energy,:time)";
    ...
    public static function getLast($esp_id, $limit = 1, $columns = '*')
    {
        ...
        $sql = "SELECT $columns FROM '3_day' ORDER BY time DESC LIMIT {$limit}";
