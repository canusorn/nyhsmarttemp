...
class Data_5_hr
{
    ...
    {
        $this->pdo = new \PDO("sqlite:" . $_SERVER['DOCUMENT_ROOT'] . "/db/data_5_hr.db");
    }
...
'CREATE INDEX IF NOT EXISTS time_index ON \'' . $this->esp_id . '\' (time);'
    ...
    {
        $pdo = new \PDO("sqlite:" . $_SERVER['DOCUMENT_ROOT'] . "/db/data_5_hr.db");
    ...
    public static function getAvDay($esp_id, $time){
        $pdo = new \PDO("sqlite:" . $_SERVER['DOCUMENT_ROOT'] . "/db/data_5_hr.db"); //valve in min
        $sql = "SELECT printf('%.1f',avg(humid)),printf('%.1f',avg(temp)),printf('%d',avg(vbatt)),SUM(valve)
                FROM '{$esp_id}' 
