...
class Data_2_sec_v10
{
    ...
    {
        $commands = [
            'CREATE TABLE IF NOT EXISTS \'2_sec\'(
                        	            pm1	REAL,
	                                    pm2	REAL,
	                                    pm10 REAL,
	                                    time TEXT NOT NULL UNIQUE)',
            'CREATE INDEX IF NOT EXISTS time_index ON \'2_sec\' (time);'
        ];
        //var_dump($commands);
    ...
    {
        $sql = "INSERT INTO '2_sec' VALUES(:pm1,:pm2,:pm10,:time)";
    ...
    {
        $pdo = new \PDO("sqlite:" . $_SERVER['DOCUMENT_ROOT'] . "/db/$esp_id.db");
        $sql = "SELECT $columns FROM '2_sec' ORDER BY time DESC LIMIT 1";
    ...
    {
        $pdo = new \PDO("sqlite:" . $_SERVER['DOCUMENT_ROOT'] . "/db/$esp_id.db");
        $sql = "SELECT $columns FROM '2_sec' ORDER BY time DESC LIMIT {$limit}";
        $stmt = $pdo->prepare($sql);
