...
class Data_5_min
{
    ...
    {
        $this->pdo = new \PDO("sqlite:" . $_SERVER['DOCUMENT_ROOT'] . "/db/data_5_min.db");
    }
    ...
    {
        $pdo = new \PDO("sqlite:" . $_SERVER['DOCUMENT_ROOT'] . "/db/data_5_min.db");
    ...
    {
        $pdo = new \PDO("sqlite:" . $_SERVER['DOCUMENT_ROOT'] . "/db/data_5_min.db"); //valve in min
        $sql = "SELECT printf('%.1f',avg(humid)),printf('%.1f',avg(temp)),printf('%d',avg(vbatt)),round(SUM(valve)/cast(60 as float),5)
                FROM '{$esp_id}' 
