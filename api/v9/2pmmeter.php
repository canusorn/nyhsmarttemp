...
if ($interval_min && $needsecupdate) {
    $data_min = new Data_2_min($_POST['esp_id']);
    $data_min->createTables();

    $data_sec_av = Data_2_sec::getAvMin($data_sec->esp_id, $lastdatetime->format('Y-m-d H:i'));
    // var_dump($data_sec_av);
    // if (!is_null($data_sec_av[0])) {
        $data_min->time = $lastdatetime->format('Y-m-d H:i');
        $data_min->pm1 = $data_sec_av[0];
        $data_min->pm2 = $data_sec_av[1];
        $data_min->pm10 = $data_sec_av[2];

        if (($re = $data_min->insert()) === true) {
            $payload["status"]["min"] = 'new min data';
        } else {
            $payload["error"][] = '[2pmmeter min], ' .  $data_min->time . ', error:' . json_encode($re);
        }
    // }
    ...
    if ($interval_hour) {
        ...
        $data_min_av = Data_2_min::getAvHr($data_sec->esp_id, $lastdatetime->format('Y-m-d H'));
        ...
        if (
            ...
            Data_2_min::deleteOldData($data_sec->esp_id);
