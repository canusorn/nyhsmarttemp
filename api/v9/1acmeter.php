...
if ($interval_min && $needsecupdate) {
    $data_min = new Data_1_min($_POST['esp_id']);
    $data_min->createTables();

    $data_sec_av = Data_1_sec::getAvMin($data_sec->esp_id, $lastdatetime->format('Y-m-d H:i'));
    ...
    if (!is_null($data_sec_av[0]) && $data_sec_av[0] >= 50) {
        ...
        $data_min->frequency = $data_sec_av[4];
        $data_min->pf = $data_sec_av[5];
    ...
    if ($interval_hour) {
        ...
        $data_min_av = Data_1_min::getAvHr($data_sec->esp_id, $lastdatetime->format('Y-m-d H'));
        ...
        if (
            ...
            Data_1_min::deleteOldData($data_sec->esp_id);
