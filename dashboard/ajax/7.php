    ...
    try {
        if (isset($_REQUEST['point'])) {
            if (isset($_REQUEST['data']) && $_REQUEST['data'] == "sec") {
                ...
                $date = new Datetime();
                $min_energy = Data_7_sec::getMinEnergy($esp_id, $date->format('Y-m-d'));
                $db = new Database();
                $conn = $db->getConn();
                $lastupdate = Esp_ID::getByESPID($conn, $esp_id)->lastupdate;
                try {
                    $last30day = Data_7_day::getLastCostom($esp_id, "1 months", "SUM(energy)");
                } catch (Throwable $e) {
                }
            ...
            // if (isset($_REQUEST['data']) && $_REQUEST['data'] == "sec") {
            //     $results = Data_7_sec::getRange($esp_id, $_REQUEST['point']);
            ...
            if (isset($_REQUEST['data']) && $_REQUEST['data'] == "sec") {
                if (isset($_REQUEST['range']['start2'])) {
                    ...
                    $results2 = Data_7_sec::getRange($esp_id, $_REQUEST['range']['start2'], $_REQUEST['range']['end2']);
                } else if (isset($_REQUEST['range']['start']))
                    $results = Data_7_sec::getRange($esp_id, $_REQUEST['range']['start'], $_REQUEST['range']['end']);
            }  else if (isset($_REQUEST['data']) && $_REQUEST['data'] == "hr") {
                if (isset($_REQUEST['range']['start2'])) {
                    $results = Data_7_hr::getRange($esp_id, $_REQUEST['range']['start'], $_REQUEST['range']['end']);
                    $results2 = Data_7_hr::getRange($esp_id, $_REQUEST['range']['start2'], $_REQUEST['range']['end2']);
                } else if (isset($_REQUEST['range']['start']))
                    $results = Data_7_hr::getRange($esp_id, $_REQUEST['range']['start'], $_REQUEST['range']['end']);
            } else if (isset($_REQUEST['data']) && $_REQUEST['data'] == "day") {
                if (isset($_REQUEST['range']['start2'])) {
                    $results = Data_7_day::getRange($esp_id, $_REQUEST['range']['start'], $_REQUEST['range']['end']);
                    $results2 = Data_7_day::getRange($esp_id, $_REQUEST['range']['start2'], $_REQUEST['range']['end2']);
                } else if (isset($_REQUEST['range']['start']))
                    $results = Data_7_day::getRange($esp_id, $_REQUEST['range']['start'], $_REQUEST['range']['end']);
            }
        }
...
if (isset($results2)) {
    ...
    $time = [];
    foreach ($results as $result) {
        $voltage[] = number_format((float)$result['v1'], 2, '.', '');
        $current[] = number_format((float)$result['c1'], 2, '.', '');
        $power[] = number_format((float)$result['p1'], 1, '.', '');
        $energy[] = number_format((float)$result['e1'], 3, '.', '');
        $voltage2[] = number_format((float)$result['v2'], 2, '.', '');
        $current2[] = number_format((float)$result['c2'], 2, '.', '');
        $power2[] = number_format((float)$result['p2'], 1, '.', '');
        $energy2[] = number_format((float)$result['e2'], 3, '.', '');
        $time[] = $result['time'];
    }
    $data = array(
        "voltage" => $voltage, "current" => $current, "power" => $power, "energy" => $energy,
        "voltage2" => $voltage2, "current2" => $current2, "power2" => $power2, "energy2" => $energy2, "time" => $time
    );
    if (isset($min_energy)) {
        $data += ["min_energy1" => $min_energy['min(e1)'],"min_energy2" => $min_energy['min(e2)']];
    }
